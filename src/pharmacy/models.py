...
class DosageGuidelines(BaseModelWithUID):
    """How to use the Drug. When have to use."""
...
class Pharmacology(BaseModelWithUID):
    """Information about how the body processes and eliminates the drug"""
...
class GuidelinesForChildren(BaseModelWithUID):
    ...
    guideline = models.CharField(max_length=100)

    def __str__(self) -> str:
        return f"{self.age_group}"


class DrugInteraction(BaseModelWithUID):
    """Explanation of how the drugs interact"""
...
class StorageInstructions(BaseModelWithUID):
    """Conditions or situations where the drug should not be used"""
...
class Drug(BaseModelWithUID):
    name = models.CharField(
        max_length=100, unique=True, help_text="This is the medicine name."
    )
    slug = AutoSlugField(populate_from=get_drug_slug, unique=True)
    ...
    drug_interaction = models.ForeignKey(
        DrugInteraction, on_delete=models.RESTRICT, null=True, blank=True
    )
    guidelines_for_children = models.ForeignKey(
        GuidelinesForChildren, on_delete=models.RESTRICT, null=True, blank=True
    )
    pharmacology = models.ForeignKey(Pharmacology, on_delete=models.RESTRICT, null=True, blank=True)
